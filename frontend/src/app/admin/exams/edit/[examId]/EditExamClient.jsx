...
export default function EditExamPage() {
  ...
  const editExamId = params?.examId;
  ...
  const [isPublishedExam, setIsPublishedExam] = useState(false);
  ...
  const fetchExamDetails = async () => {
    try {
      ...
      setExamForm({
        ...
        chId: exam.chId ? String(exam.chId) : "",
        examTitle: exam.examTitle || "",
        examDesc: exam.examDesc || "",
        publishMode: exam.publishMode || "manual",
        scheduledPublishAt: formatDateTimeLocal(exam.scheduledPublishAt),
        durationMinutes: exam.durationMinutes || 30,
        totalMarks: exam.totalMarks || 0,
        passingMarks: exam.passingMarks || 0,
        maxAttempts: exam.maxAttempts || 1,
        requireCompletion: Number(exam.requireCompletion ?? 1),
        completionPercent: exam.completionPercent || 100,
        accessType: exam.accessType || "all_course_students",
        checkingType: Number(exam.checkingType || 0),
        isPublished: Number(exam.isPublished || 0),
      });
  ...
  const DRAFT_KEY = isEditMode
    ? `edit_exam_draft_${editExamId}`
    : "create_exam_draft";
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setSelectedType(draft.selectedType || "multiple");
        setExamId(draft.examId || null);
        setCurrentStep(draft.currentStep || 1);
        setExamDetailsSaved(draft.examDetailsSaved || false);
        setAccessRulesSaved(draft.accessRulesSaved || false);
        setExamForm(
          draft.examForm || {
            examType: "course",
            courseId: "",
  ...
  const saveStepOneExamDetails = async () => {
    const duplicateExam = !isEditMode ? checkDuplicateExam() : null;
  ...
  const groupedQuestions = useMemo(() => {
    const typeMap = questionTypes.reduce((acc, type) => {
      acc[type.key] = {
        ...type,
        questions: [],
      };
    ...
    questions.forEach((q) => {
      if (typeMap[q.questionType]) {
        typeMap[q.questionType].questions.push(q);
      }
    });

    const typeOrderFromQuestions = [];

    questions.forEach((q) => {
      if (!typeOrderFromQuestions.includes(q.questionType)) {
        typeOrderFromQuestions.push(q.questionType);
      }
    });

    return typeOrderFromQuestions
      .map((type) => typeMap[type])
      .filter((group) => group && group.questions.length > 0);
  }, [questions]);

  const saveQuestionSequence = async (updatedQuestions) => {
    try {
      const sequenceData = updatedQuestions.map((q, index) => ({
  ...
  const handleQuestionDrop = async (targetQuestionId) => {
    ...
    const targetIndex = questions.findIndex(
      (q) => q.questionId === targetQuestionId,
    );
    ...
    await saveQuestionSequence(updated);
  };

  const handleTypeDrop = async (targetType) => {
    if (!draggedType || draggedType === targetType) return;

    const currentTypeOrder = groupedQuestions.map((group) => group.key);

    const draggedIndex = currentTypeOrder.indexOf(draggedType);
    const targetIndex = currentTypeOrder.indexOf(targetType);

    if (draggedIndex === -1 || targetIndex === -1) return;

    const newTypeOrder = [...currentTypeOrder];
    const [movedType] = newTypeOrder.splice(draggedIndex, 1);
    newTypeOrder.splice(targetIndex, 0, movedType);

    const updatedQuestions = newTypeOrder.flatMap((type) =>
      questions.filter((q) => q.questionType === type),
    );
    ...
    await saveQuestionSequence(updatedQuestions);
  };
          ...
          <div className="rounded-3xl bg-white p-6 shadow-lg">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <FormSelect
                ...
                onChange={(e) => {
                  updateExamDetailsForm("examType", e.target.value);
                        ...
                        <div className="space-y-4">
                          {filteredQuestions.map((q, index) => (
                            <QuestionCard
                              ...
                              onDragStart={(e) => {
                                e.stopPropagation();
                                setDraggedQuestionId(q.questionId);
                                setDraggedType(null);
                              }}
                              ...
                              onDrop={(e) => {
                                ...
                                if (draggedQuestionId) {
                                  handleQuestionDrop(q.questionId);
                                }
                              }}
                            />
                          ))}
                            ...
                            return (
                              ...
                              >
                                <div
                                  ...
                                  className="mb-4 flex cursor-grab items-center gap-3 rounded-2xl bg-white p-4 shadow-sm active:cursor-grabbing"
                                >
                                  <Icon className="h-5 w-5 text-purple-700" />

                                  <h3 className="text-lg font-black text-purple-800">
                                    {group.title}
                                  </h3>
...
function QuestionCard({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className="cursor-grab rounded-2xl border border-gray-200 bg-white p-4 active:cursor-grabbing"
    >

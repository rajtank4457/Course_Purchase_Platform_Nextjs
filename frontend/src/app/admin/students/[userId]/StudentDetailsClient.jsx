...
export default function StudentDetailsPage() {
      ...
      const uniqueChapters = Array.from(
        new Map(
          data.map((chapter) => [
            ...
            {
              ...chapter,
              progress: Math.round(Number(chapter.progress || 0)),
              notesProgress: Math.round(Number(chapter.notesProgress || 0)),
              videoProgress: Math.round(Number(chapter.videoProgress || 0)),
              sourceProgress: Math.round(Number(chapter.sourceProgress || 0)),
            },
          ]),
        ).values(),
      );
  ...
  const resetSingleChapterProgress = async (chapter) => {
    if (!confirm(`Reset progress for "${chapter.chapterName}"?`)) return;
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-3xl bg-white p-5 shadow">
            ...
            <h2 className="mt-2 text-4xl font-black text-blue-600">
              {
                courses.filter((item) => Number(item.progress || 0) === 100)
                  .length
              }
            </h2>
          </div>
                    ...
                    chapters.map((chapter) => (
                      ...
                      >
                        ...
                        <div className="mt-3 grid gap-2 text-xs font-bold text-gray-500 sm:grid-cols-4">
                          <span>
                            Desc: {chapter.descDone ? "Done" : "Pending"}
                          </span>
                        ...
                        <div className="mt-4">
                          <button
                            disabled={actionLoading}
                            onClick={() => resetSingleChapterProgress(chapter)}
                            className="inline-flex items-center gap-1 rounded-xl bg-orange-100 px-3 py-2 text-sm font-black text-orange-700 hover:bg-orange-200 disabled:opacity-60"
                          >
                            <RotateCcw className="h-4 w-4" />
                            Reset Chapter

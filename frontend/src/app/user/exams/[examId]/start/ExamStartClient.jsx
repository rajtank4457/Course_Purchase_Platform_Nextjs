...
export default function ExamStartClient() {
  ...
  const fetchExamInfo = async () => {
    try {
      const service = examApi.getExamStartInfo(examId);
      ...
      if (!res.success) {
        alert(res.message || "Exam not available");
  ...
  const startExam = async () => {
    try {
      ...
      if (!res.success) {
        alert(res.message || "Cannot start exam");
        return;
      }

      router.push(
        `/user/exams/${examId}/attempt?attemptId=${res.data?.attemptId || res.data?.data?.attemptId}`,
      );
    } catch (err) {
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="rounded-3xl bg-white p-8 shadow-xl">
          ...
          <div className="mt-8 rounded-3xl bg-purple-50 p-6">
            <h2 className="text-xl font-black text-purple-900">Instructions</h2>
